...
export default function ContactFooter() {
  ...
  // Konfigurasi Intersection Observer untuk meriset animasi bolak-balik (keluar masuk viewport)
  useEffect(() => {
    const observer = new IntersectionObserver(
      ...
      {
        threshold: 0.1, // Terpicu ketika 10% komponen mulai masuk layar
      }
    );
  ...
  return (
    <footer id="contact" ref={sectionRef} className="pt-32 bg-dark-bg border-t border-white/5 relative">
      <div className="max-w-7xl mx-auto px-6 flex flex-col items-center">
        
        {/* Centered CTA Block */}
        <div className="flex flex-col items-center text-center mb-16 w-full">
          {/* 1. Pill Label: Smooth Blur Fade-Up */}
          <div className={`px-4 py-1.5 rounded-full border border-cyan-neon bg-cyan-neon/10 text-cyan-neon text-xs font-bold tracking-widest mb-8 transition-all duration-700 ease-out ${
            isVisible ? "opacity-100 translate-y-0 blur-none" : "opacity-0 translate-y-6 blur-md"
          }`}>
            ✦ AVAILABLE FOR OPPORTUNITIES
          </div>

          {/* 1. Heading: Smooth Blur Fade-Up (Delay 100ms) */}
          <h2 className={`font-unbounded text-5xl md:text-7xl lg:text-8xl font-black leading-[1.1] mb-6 uppercase transition-all duration-700 ease-out ${
            isVisible ? "opacity-100 translate-y-0 blur-none delay-100" : "opacity-0 translate-y-6 blur-md"
          }`}>
            BRINGING IDEAS <br />
            INTO <span className="bg-gradient-to-r from-cyan-neon to-purple-neon bg-clip-text text-transparent">REALITY</span>
          </h2>

          {/* 1. Description Text: Smooth Blur Fade-Up (Delay 200ms) */}
          <p className={`max-w-xl text-text-muted text-sm md:text-base mb-10 leading-relaxed transition-all duration-700 ease-out ${
            isVisible ? "opacity-100 translate-y-0 blur-none delay-200" : "opacity-0 translate-y-6 blur-md"
          }`}>
            Whether it's a completely new system build, strategic component engineering, or layout consultation, my inbox is open.
          </p>
          
          {/* Pembungkus tombol email untuk mengisolasi efek transisi scroll masuk dengan efek transisi hover asli */}
          <div className={`transition-all duration-700 ease-out ${
            isVisible ? "opacity-100 scale-100 delay-400" : "opacity-0 scale-95"
          }`}>
            {/* 2. Main Email Button: Tetap menjaga style asli hover bg-gray-200 */}
            <a 
              href="mailto:hamdaniachmad480@gmail.com" 
              className="inline-block bg-white text-dark-bg rounded-full px-10 py-5 text-xl font-bold hover:bg-gray-200 transition-colors duration-300 shadow-[0_0_40px_-5px_#ffffff]"
            >
              hamdaniachmad480@gmail.com
            </a>
          </div>

          {/* 3. Location Badge: Staggered Slide-Up (Delay 500ms) */}
          <div className={`inline-flex items-center gap-2 text-xs font-semibold text-text-muted bg-dark-card px-4 py-2 rounded-full border border-white/5 mt-8 tracking-wide transition-all duration-400 ease-out ${
            isVisible ? "opacity-100 translate-y-0 delay-500" : "opacity-0 translate-y-4"
          }`}>
        ...
        {/* 3. Social Media Cards: Diisolasi transisi masuknya menggunakan inline style transitionDelay agar tidak merusak durasi hover */}
        <div className="w-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-8 mb-24">
          {socials.map((social, index) => {
            const delayInMs = 600 + (index * 100);
            ...
            return (
              ...
              >
                {/* Elemen Link Asli: Efek hover disinkronkan total menggunakan transition-colors duration-300 normal */}
                <a
                  href={social.link}
                  target="_blank"
                ...
                >
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 rounded-full bg-dark-bg border border-white/10 flex items-center justify-center text-text-muted group-hover:text-white transition-colors duration-300">
                      {social.svg}
                    </div>
                    <div>
      ...
      {/* 4. Footer Area: Simple Fade-In statis di tempat tanpa pergeseran posisi */}
      <div className={`border-t border-white/5 py-10 bg-dark-bg transition-opacity duration-1000 ease-out delay-1000 ${
        ...
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-3 gap-8 items-center text-center md:text-left">
          ...
          <div className="flex flex-wrap justify-center gap-6">
            <a href="#home" className="text-xs font-bold uppercase tracking-widest text-text-muted hover:text-cyan-neon transition-colors duration-300">Home</a>
            <a href="#about" className="text-xs font-bold uppercase tracking-widest text-text-muted hover:text-cyan-neon transition-colors duration-300">About</a>
            <a href="#projects" className="text-xs font-bold uppercase tracking-widest text-text-muted hover:text-cyan-neon transition-colors duration-300">Project</a>
            <a href="#contact" className="text-xs font-bold uppercase tracking-widest text-text-muted hover:text-cyan-neon transition-colors duration-300">Contact</a>
          </div>
